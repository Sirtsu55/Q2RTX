...
void P_DamageFeedback(edict_t *player)
{
    ...
    if ((level.framenum > player->pain_debounce_framenum) && !(player->flags & FL_GODMODE) && (client->invincible_framenum <= level.framenum)) {
        ...
        if (player->health < 25)
            ...
            l = 100;
        gi.sound(player, CHAN_VOICE, SV_SoundIndex(va("*pain%i_%i.wav", l, r)), 1, ATTN_NORM, 0);
    }
...
void SV_CalcBlend(edict_t *ent)
{
    ...
    if (ent->client->quad_framenum > level.framenum) {
        ...
        if (remaining == 30)    // beginning to fade
            gi.sound(ent, CHAN_ITEM, SV_SoundIndex("items/damage2.wav"), 1, ATTN_NORM, 0);
        if (remaining > 30 || (remaining & 4))
            SV_AddBlend(0, 0, 1, 0.08f, ent->client->ps.blend);
    } else if (ent->client->invincible_framenum > level.framenum) {
        remaining = ent->client->invincible_framenum - level.framenum;
        if (remaining == 30)    // beginning to fade
            gi.sound(ent, CHAN_ITEM, SV_SoundIndex("items/protect2.wav"), 1, ATTN_NORM, 0);
        if (remaining > 30 || (remaining & 4))
            SV_AddBlend(1, 1, 0, 0.08f, ent->client->ps.blend);
    } else if (ent->client->enviro_framenum > level.framenum) {
        remaining = ent->client->enviro_framenum - level.framenum;
        if (remaining == 30)    // beginning to fade
            gi.sound(ent, CHAN_ITEM, SV_SoundIndex("items/airout.wav"), 1, ATTN_NORM, 0);
        if (remaining > 30 || (remaining & 4))
...
void P_WorldEffects(void)
{
    ...
    if (!old_waterlevel && waterlevel) {
        ...
        if (current_player->watertype & CONTENTS_LAVA)
            gi.sound(current_player, CHAN_BODY, SV_SoundIndex("player/lava_in.wav"), 1, ATTN_NORM, 0);
        else if (current_player->watertype & CONTENTS_SLIME)
            gi.sound(current_player, CHAN_BODY, SV_SoundIndex("player/watr_in.wav"), 1, ATTN_NORM, 0);
    ...
    if (old_waterlevel && ! waterlevel) {
        PlayerNoise(current_player, current_player->s.origin, PNOISE_SELF);
        gi.sound(current_player, CHAN_BODY, SV_SoundIndex("player/watr_out.wav"), 1, ATTN_NORM, 0);
        current_player->flags &= ~FL_INWATER;
    }
    ...
    if (old_waterlevel != 3 && waterlevel == 3) {
        gi.sound(current_player, CHAN_BODY, SV_SoundIndex("player/watr_un.wav"), 1, ATTN_NORM, 0);
    }
    ...
    if (old_waterlevel == 3 && waterlevel != 3) {
        if (current_player->air_finished_framenum < level.framenum) {
            // gasp for air
            gi.sound(current_player, CHAN_VOICE, SV_SoundIndex("player/gasp1.wav"), 1, ATTN_NORM, 0);
            PlayerNoise(current_player, current_player->s.origin, PNOISE_SELF);
        } else  if (current_player->air_finished_framenum < level.framenum + 11 * BASE_FRAMERATE) {
            // just break surface
            gi.sound(current_player, CHAN_VOICE, SV_SoundIndex("player/gasp2.wav"), 1, ATTN_NORM, 0);
        }
    }
        ...
        if (breather || envirosuit) {
            ...
            if (((int)(current_client->breather_framenum - level.framenum) % 25) == 0) {
                if (!current_client->breather_sound)
                    gi.sound(current_player, CHAN_AUTO, SV_SoundIndex("player/u_breath1.wav"), 1, ATTN_NORM, 0);
                else
                    gi.sound(current_player, CHAN_AUTO, SV_SoundIndex("player/u_breath2.wav"), 1, ATTN_NORM, 0);
        ...
        if (current_player->air_finished_framenum < level.framenum) {
            ...
            if (current_player->client->next_drown_framenum < level.framenum
                ...
                if (current_player->health <= current_player->dmg)
                    gi.sound(current_player, CHAN_VOICE, SV_SoundIndex("player/drown1.wav"), 1, ATTN_NORM, 0);
                else if (Q_rand() & 1)
                    gi.sound(current_player, CHAN_VOICE, SV_SoundIndex("*gurp1.wav"), 1, ATTN_NORM, 0);
                else
                    gi.sound(current_player, CHAN_VOICE, SV_SoundIndex("*gurp2.wav"), 1, ATTN_NORM, 0);
    ...
    if (waterlevel && (current_player->watertype & (CONTENTS_LAVA | CONTENTS_SLIME))) {
        if (current_player->watertype & CONTENTS_LAVA) {
            if (current_player->health > 0
                ...
                if (Q_rand() & 1)
                    gi.sound(current_player, CHAN_VOICE, SV_SoundIndex("player/burn1.wav"), 1, ATTN_NORM, 0);
                else
                    gi.sound(current_player, CHAN_VOICE, SV_SoundIndex("player/burn2.wav"), 1, ATTN_NORM, 0);
...
void G_SetClientSound(edict_t *ent)
{
    ...
    if (ent->client->pers.helpchanged && ent->client->pers.helpchanged <= 1 && !(level.framenum & 63)) {
        ...
        gi.sound(ent, CHAN_VOICE, SV_SoundIndex("misc/pc_up.wav"), 1, ATTN_STATIC, 0);
    }
