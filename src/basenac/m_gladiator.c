...
{
    SV_StartSound(self, CHAN_WEAPON, sound_cleaver_swing, 1, ATTN_NORM, 0);
}
...
void GaldiatorMelee(edict_t *self)
{
    ...
    if (fire_hit(self, aim, (20 + (Q_rand() % 5)), 300))
        SV_StartSound(self, CHAN_AUTO, sound_cleaver_hit, 1, ATTN_NORM, 0);
    else
        SV_StartSound(self, CHAN_AUTO, sound_cleaver_miss, 1, ATTN_NORM, 0);
}
...
void gladiator_attack(edict_t *self)
{
    ...
    SV_StartSound(self, CHAN_WEAPON, sound_gun, 1, ATTN_NORM, 0);
