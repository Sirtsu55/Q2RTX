...
{
    SV_StartSound(self, CHAN_BODY, sound_sight, 1, ATTN_NORM, 0);
}
...
// regular death
    ...
    if (n == 0) {
        ...
        SV_StartSound(self, CHAN_VOICE, sound_die2, 1, ATTN_NORM, 0);
    } else if (n == 1) {
        self->monsterinfo.currentmove = &infantry_move_death2;
        SV_StartSound(self, CHAN_VOICE, sound_die1, 1, ATTN_NORM, 0);
    } else {
...
{
    SV_StartSound(self, CHAN_WEAPON, sound_punch_swing, 1, ATTN_NORM, 0);
}
...
void infantry_smack(edict_t *self)
{
    ...
    if (fire_hit(self, aim, (5 + (Q_rand() % 5)), 50))
        SV_StartSound(self, CHAN_WEAPON, sound_punch_hit, 1, ATTN_NORM, 0);
}
