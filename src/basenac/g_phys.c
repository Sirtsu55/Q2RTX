...
// check for water transition
    ...
    if (!wasinwater && isinwater)
        gi.positioned_sound(old_origin, g_edicts, CHAN_AUTO, gi.soundindex("misc/h2ohit1.wav"), 1, ATTN_NORM);
    else if (wasinwater && !isinwater)
        gi.positioned_sound(ent->s.origin, g_edicts, CHAN_AUTO, gi.soundindex("misc/h2ohit1.wav"), 1, ATTN_NORM);
...
void SV_Physics_Step(edict_t *ent)
{
    ...
    if (ent->velocity[2] || ent->velocity[1] || ent->velocity[0]) {
        ...
        if (ent->groundentity)
            if (!wasonground)
                if (hitsound)
                    gi.sound(ent, 0, gi.soundindex("world/land.wav"), 1, ATTN_NORM);
    }
