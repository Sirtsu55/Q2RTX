...
void medic_idle(edict_t *self)
{
    ...
    SV_StartSound(self, CHAN_VOICE, sound_idle1, 1, ATTN_IDLE, 0);
...
void medic_search(edict_t *self)
{
    ...
    SV_StartSound(self, CHAN_VOICE, sound_search, 1, ATTN_IDLE, 0);
...
{
    SV_StartSound(self, CHAN_WEAPON, sound_hook_launch, 1, ATTN_NORM, 0);
}
...
void medic_cable_attack(edict_t *self)
{
    ...
    if (self->s.frame == FRAME_attack43) {
        SV_StartSound(self->enemy, CHAN_AUTO, sound_hook_hit, 1, ATTN_NORM, 0);
        ...
        if (self->s.frame == FRAME_attack44)
            SV_StartSound(self, CHAN_WEAPON, sound_hook_heal, 1, ATTN_NORM, 0);
    }
...
{
    SV_StartSound(self, CHAN_WEAPON, sound_hook_retract, 1, ATTN_NORM, 0);
    self->enemy->monsterinfo.aiflags &= ~AI_RESURRECTING;
}
