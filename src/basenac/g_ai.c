...
void ai_stand(edict_t *self, float dist)
{
    ...
    if (!(self->spawnflags & 1) && (self->monsterinfo.idle) && (level.time > self->monsterinfo.idle_time)) {
        if (self->monsterinfo.idle_time) {
            ...
            self->monsterinfo.idle_time = level.time + G_SecToMs((1 + random()) * 15);
        } else {
            self->monsterinfo.idle_time = level.time + G_SecToMs(random() * 15);
        }
    }
}
...
int range(edict_t *self, edict_t *other)
{
    ...
    len = VectorLength(v);
    if (len < self->monsterinfo.melee_distance)
        return RANGE_MELEE;
    if (len < 500)
        return RANGE_NEAR;
    if (len < 1000)
...
void FoundTarget(edict_t *self)
{
    ...
    self->show_hostile_time = level.time + 1000;   // wake up other monsters
    ...
    if (!self->movetarget) {
        ...
        HuntTarget(self);
        Com_WPrintf("%s at %s, combattarget %s not found\n", self->classname, vtos(self->s.origin), self->combattarget);
        return;
    }
...
// but not weapon impact/explosion noises
    ...
    heardit = false;
    if ((level.sight_entity_time >= (level.time - BASE_FRAMETIME)) && !(self->spawnflags & 1)) {
        client = level.sight_entity;
        if (client->enemy == self->enemy) {
            return false;
        }
    } else if (level.sound_entity_time >= (level.time - BASE_FRAMETIME)) {
        client = level.sound_entity;
        heardit = true;
    } else if (!(self->enemy) && (level.sound2_entity_time >= (level.time - BASE_FRAMETIME)) && !(self->spawnflags & 1)) {
...
// this causes monsters to run blindly to the combat point w/o firing
    if (self->goalentity) {
        ...
        if (self->monsterinfo.aiflags & AI_SOUND_TARGET) {
            if ((level.time - self->enemy->last_sound_time) > 5000) {
                ...
                self->show_hostile_time = level.time + 1000;
                return false;
            }
        }
    }
