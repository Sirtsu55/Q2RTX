...
{
    SV_StartSound(self, CHAN_AUTO, sound_idle3, 1, ATTN_IDLE, 0);
...
void brain_dodge(edict_t *self, edict_t *attacker, float eta) {
    ...
    self->monsterinfo.pause_time = level.time + G_SecToMs(eta + 0.5f);
...
void brain_swing_right(edict_t *self) {
    SV_StartSound(self, CHAN_BODY, sound_melee1, 1, ATTN_NORM, 0);
}
...
void brain_hit_right(edict_t *self) {
    ...
    if (fire_hit(self, aim, (15 + (Q_rand() % 5)), 40))
        SV_StartSound(self, CHAN_WEAPON, sound_melee3, 1, ATTN_NORM, 0);
}

void brain_swing_left(edict_t *self) {
    SV_StartSound(self, CHAN_BODY, sound_melee2, 1, ATTN_NORM, 0);
}
...
void brain_tentacle_attack(edict_t *self) {
    ...
    if (fire_hit(self, aim, (10 + (Q_rand() % 5)), -600) && skill.integer > 0)
        self->spawnflags |= 65536;
    SV_StartSound(self, CHAN_WEAPON, sound_tentacles_retract, 1, ATTN_NORM, 0);
}
