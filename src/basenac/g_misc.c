...
//  gi.dprintf ("portalstate: %i = %i\n", ent->style, ent->count);
    SV_SetAreaPortalState(ent->style, ent->count);
}
...
void gib_think(edict_t *self)
{
    ...
    if (self->s.frame == 10) {
        ...
        self->nextthink = level.time + G_SecToMs(8 + random() * 10);
    }
}
...
void ThrowGib(edict_t *self, char *gibname, int damage, int type)
{
    ...
    gib->s.modelindex = SV_ModelIndex(gibname);
    gib->solid = SOLID_NOT;
    ...
    gib->nextthink = level.time + G_SecToMs(10 + random() * 10);
...
void ThrowHead(edict_t *self, char *gibname, int damage, int type)
{
    ...
    self->s.modelindex = SV_ModelIndex(gibname);
    ...
    self->think = G_FreeEdict;
    self->nextthink = level.time + G_SecToMs(10 + random() * 10);
...
void ThrowDebris(edict_t *self, char *modelname, float speed, vec3_t origin)
{
    ...
    chunk->s.modelindex = SV_ModelIndex(modelname);
    v[0] = 100 * crandom();
    v[1] = 100 * crandom();
    v[2] = 100 + 100 * crandom();
    ...
    chunk->nextthink = level.time + G_SecToMs(5 + random() * 5);
    chunk->s.frame = 0;
    chunk->flags = 0;
...
void path_corner_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
    ...
    if (self->wait) {
        other->monsterinfo.pause_time = level.time + G_SecToMs(self->wait);
...
{
    if (!self->targetname) {
        Com_WPrintf("path_corner with no targetname at %s\n", vtos(self->s.origin));
        G_FreeEdict(self);
        return;
    }
...
void point_combat_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
    ...
    if (self->target) {
        ...
        if (!other->goalentity) {
            Com_WPrintf("%s at %s target %s does not exist\n", self->classname, vtos(self->s.origin), self->target);
...
void SP_func_wall(edict_t *self)
{
    ...
    //      gi.dprintf("func_wall missing TRIGGER_SPAWN\n");
        ...
        if (self->spawnflags & 4) {
            if (!(self->spawnflags & 2)) {
                Com_WPrintf("func_wall START_ON without TOGGLE\n");
                self->spawnflags |= 2;
            }
        }
...

{
    ...
    M_walkmove(self, vectoyaw(v), 20 * ratio * BASE_FRAMETIME_S);
}
...
void commander_body_think(edict_t *self)
{
    ...
    if (self->s.frame == 22)
        SV_StartSound(self, CHAN_BODY, SV_SoundIndex("tank/thud.wav"), 1, ATTN_NORM, 0);
}
...
void commander_body_use(edict_t *self, edict_t *other, edict_t *activator)
{
    ...
    self->nextthink = level.time + 1;
    SV_StartSound(self, CHAN_BODY, SV_SoundIndex("tank/pain.wav"), 1, ATTN_NORM, 0);
}
...
{
    if (!ent->target) {
        Com_WPrintf("misc_viper without a target at %s\n", vtos(ent->absmin));
        G_FreeEdict(ent);
        return;
    }
...
void misc_viper_bomb_prethink(edict_t *self)
{
    ...
    diff = G_MsToSec(self->timestamp - level.time);
    if (diff < -1.0f)
        diff = -1.0f;
...
{
    if (!ent->target) {
        Com_WPrintf("%s without a target at %s\n", ent->classname, vtos(ent->absmin));
        G_FreeEdict(ent);
        return;
    }
...
{
    ent->s.modelindex = SV_ModelIndex("models/objects/gibs/arm/tris.md2");
    ent->solid = SOLID_NOT;
...
{
    ent->s.modelindex = SV_ModelIndex("models/objects/gibs/leg/tris.md2");
    ent->solid = SOLID_NOT;
    ent->s.effects |= EF_GIB;
    ent->takedamage = DAMAGE_YES;
    ent->die = gib_die;
...
{
    ent->s.modelindex = SV_ModelIndex("models/objects/gibs/head/tris.md2");
    ent->solid = SOLID_NOT;
    ent->s.effects |= EF_GIB;
    ent->takedamage = DAMAGE_YES;
    ent->die = gib_die;
...
void SP_func_clock(edict_t *self)
{
    ...
    if ((self->spawnflags & 2) && (!self->count)) {
        Com_WPrintf("%s with no count at %s\n", self->classname, vtos(self->s.origin));
        G_FreeEdict(self);
        return;
    }
...
void teleporter_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
    ...
    if (!dest) {
        Com_WPrintf("Couldn't find destination \"%s\"\n", self->target);
        return;
    }
...
void SP_misc_teleporter(edict_t *ent)
{
    ...
    ent->s.modelindex = SV_ModelIndex("models/objects/dmspot/tris.md2");
    ent->s.skinnum = 1;
